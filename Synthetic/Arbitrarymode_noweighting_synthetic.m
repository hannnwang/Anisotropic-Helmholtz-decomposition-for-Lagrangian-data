%This corresponds to the traditional unweighted approach, mentioned in section 3 in paper..
%Programming-wise, the only difference between this and
%Arbitrarymode_angleweighted_trapezoidal_synthetic.m is that this program
%replaces lines 137-197 in
%Arbitrarymode_angleweighted_trapezoidal_synthetic.m with a simple  unweighted ensemble
%averaging.
...
for ix=1:length(dist_axis)
    ...
    %structure functions: unweighted average in this approach.
    dll_M_cos_ensemble(ix)=mean(dll_M_cos_v);
    dll_M_sin_ensemble(ix)=mean(dll_M_sin_v);
    dtt_M_cos_ensemble(ix)=mean(dtt_M_cos_v);
    dtt_M_sin_ensemble(ix)=mean(dtt_M_sin_v);
    dlt_M_cos_ensemble(ix)=mean(dlt_M_cos_v);
    dlt_M_sin_ensemble(ix)=mean(dlt_M_sin_v);
