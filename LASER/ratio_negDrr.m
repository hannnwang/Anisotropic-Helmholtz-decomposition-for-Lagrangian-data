...
cd 'D:\OBresearch\Project_aniso\LASER\20200214\circle4\trapezoidal'
  Moment=0;
...
Ddd_2D_plot=Ddd_2D;
Ddd_2D_plot(Drr_2D>=0)=NaN;
ratio_bug_2D=abs(Drr_2D_plot)./(abs(Drr_2D_plot)+abs(Ddd_2D_plot));
...
hold on;
contourf(Xlog,Ylog,ratio_bug_2D,10,'edgecolor','none')
...
R=max(max(ratio_bug_2D(~isnan(ratio_bug_2D))))
(1-R)/R